...
Open the URL the dev server prints (<http://127.0.0.1:5173>). Beneath its own banner it prints
the origin it proxies `/api` to; if that is not where the API is listening, set
...
- **"nothing answered for the API"** — the dev server could not reach the API. Check the `/api →`
  line it printed against the address the API says it is listening on.
