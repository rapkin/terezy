"""The belief a carried quotation rests on, and every refusal it owes.
...
DECLARED = DATA_ROOT / "scenarios" / "quotation" / "owner-001.toml"
...
def test_the_shipped_belief_loads() -> None:
    owner_id, assumption = loader.quotation_belief_from_file(DECLARED)
...
def test_an_absent_directory_is_refused(tmp_path: Path) -> None:
    ...
    (root / "scenarios" / "quotation" / "owner-001.toml").unlink()
    with pytest.raises(DeclarationError) as caught:
        _resolve(root)
    assert caught.value.file == root / "scenarios" / "quotation"
...
def test_two_declared_beliefs_are_refused(tmp_path: Path) -> None:
    ...
    root = _scratch_root(tmp_path)
    shutil.copy2(DECLARED, root / "scenarios" / "quotation" / "owner-002.toml")
    with pytest.raises(DeclarationError) as caught:
        _resolve(root)
    assert caught.value.file == root / "scenarios" / "quotation"
...
def test_a_belief_belonging_to_another_owner_is_refused(tmp_path: Path) -> None:
    ...
    root = _scratch_root(tmp_path)
    target = root / "scenarios" / "quotation" / "owner-001.toml"
    target.write_text(
