...
| `user/seeds/owner-001.toml` | The **private overlay** in the one position a test can reach it: `<root>/user/seeds/`, which `resolver.data_roots_of` derives and layers over the shipped root (025 FR-001). Its lots join the shipped root's rather than replacing them. |
