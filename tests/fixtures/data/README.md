...
| `instruments/synthetic_held_x.toml` | The only **held asset** any test can reach: no coupon, no maturity, no schedule, a ticker the price series is fetched under, and a tax class no pack declares. `btc` ships no observation file by design, so without this the PRICED path of a held position would be reachable by nothing. |
| `observations/binance_synthusdt.toml` | An invented daily-close series in the shape `scripts/fetch_binance.py` writes, so a held position has a price at all. The first observation file this repository reads at run time. |
| `user/seeds/owner-001.toml` | The **private overlay** in the one position a test can reach it: `<root>/user/seeds/`, which `resolver.data_roots_of` derives and layers over the shipped root (025 FR-001). Its lots join the shipped root's rather than replacing them, and its cost is in DOLLARS -- the currency the held asset declares -- so the strike into hryvnia is exercised. |
...
| `questions/a-holding-among-the-words.toml` | The only declared question naming a **held asset**, so the answer's held section is reachable over the HTTP API at all: his own names `btc`, which has no committed lot, so every served answer reported `held: []`. Its id sorts first because the web client answers `GET /api/questions`'s first id. |
