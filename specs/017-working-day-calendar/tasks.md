...
- [ ] **T001** `tests/contract/test_no_calendar_free_working_day.py`: FR-018's scan. Parse every
...
- [ ] **T002** FR-017a: correct the three sentences that go false.
...
- [ ] **T003** Checkpoint: gates green, commit.

## Phase 1 — the core records and the four questions

- [ ] **T004** `tests/unit/test_working_day_calendar.py`, part 1: construct all three refusal
  ...
  calendar.
- [ ] **T005** `src/terezy/core/calendars/__init__.py` and `working_day.py`: the records and
  refusals of [data-model.md](./data-model.md). `pre_holiday` on `WorkingDay` only.
- [ ] **T006** `tests/worked_examples/test_working_day_classification.py`: a synthetic calendar —
...
- [ ] **T007** `working_day.classify`: bisect the rows, fall through to the declared rest pattern,
  and carry the deciding declaration's provenance onto the answer.
- [ ] **T008** `tests/unit/test_working_day_calendar.py`, part 2: the FR-012–FR-014 battery over a
...
- [ ] **T009** `first_working_day_on_or_after`, `last_working_day_on_or_before`,
  `last_working_day_of_week`.
- [ ] **T010** `tests/unit/test_working_day_calendar.py`, part 3: SC-002's out-of-window battery —
  ...
  them into an answer.
- [ ] **T011** Checkpoint: gates green, commit.

## Phase 2 — the declaration surface

- [ ] **T012** `tests/contract/test_calendar_declaration_loading.py`: SC-004's battery, every case
  ...
  default.
- [ ] **T013** `schema.py`: `CalendarFile`, `CalendarTable`, `CalendarCoverageTable`,
  `CalendarWeekTable`, `CalendarDayTable`, all `STRICT`, all fields required.
- [ ] **T014** `loader.py`: `working_day_calendar_from_file`, and the `CALENDAR_*` field-path
  constants beside it.
- [ ] **T015** `resolver.py`: `CALENDARS_DIR`, `working_day_calendars_from_data_root(root, kinds)`
...
- [ ] **T016** `scripts/check_provenance.py`: `data/calendars/` joins `SOURCED_DIRS`, with its
  reason written where the others are.
- [ ] **T017** Checkpoint: gates green, commit.

## Phase 3 — the shipped calendar, and the gate measured on it

- [ ] **T018** `data/calendars/ua_civil.toml`: the one shipped `civil` calendar. Window
...
- [ ] **T019** `tests/contract/test_provenance_gate.py`: SC-005's **measured delta**. Record the
...
- [ ] **T020** Checkpoint: gates green, commit.

## Phase 4 — the data-only claims, the no-consumer claim, and the documentation

- [ ] **T021** `tests/contract/test_calendar_data_only.py`: SC-006 — a second jurisdiction's
...
- [ ] **T022** SC-010: assert the golden suite is untouched by this feature, and that nothing in
  `src/` outside `core/calendars/` and the declaration layer imports `core.calendars`.
- [ ] **T023** `docs/METHODOLOGY.md`: a new `## 35. The working-day calendar` **before** `##
...
- [ ] **T024** `docs/REQUIRED_TESTS.md`: this feature's rows and what they do and do not close —
  H1 exercised not closed, H2 exercised, E8 not attempted.
- [ ] **T025** Verify SC-013 on this branch: `specs/features.toml`'s
...
- [ ] **T026** Final gates, `/condense`, `/code-review`, iterate to clean.
