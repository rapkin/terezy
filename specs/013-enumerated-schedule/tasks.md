...
- [x] T055 `/code-review` round three, over the diff that will actually land. Two blockers: `docs/METHODOLOGY.md` carried **two `## 31.` sections** after a text-merge with `main`, which `check_methodology_refs.py` had been failing on unread; and the F20 seal closed over `day_count` while `face_value` — the *other* term both forms declare, and the one F2 actually read — stayed open
- [ ] T056 `/code-review` round four, over the diff that will actually land. **Round three is spent**: the branch changed after it, and CLAUDE.md is explicit that a review of a diff that no longer exists is not a review of what is being merged. Round one also reported what it did *not* reach — every spec artefact, the golden file, three of the four data fixtures, and eleven test modules beyond their headers — so round two has a stated scope rather than a fresh sweep
