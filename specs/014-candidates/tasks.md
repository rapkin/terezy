...
- [ ] **T004** repair every existing site that constructs or asserts a `CompositionRefused`
      (004's suites) so the whole tree is green before anything of 014 exists.

- [ ] **T005 (test)** `tests/contract/test_candidate_ceiling_declaration.py` — the loader
...
- [ ] **T006** `src/terezy/data/declarations/schema.py` — `CandidatesTable`, `CandidatesFile`.
- [ ] **T007** `src/terezy/data/declarations/loader.py` — `candidates_from_file`, on
      `composition_from_file`'s shape exactly.
- [ ] **T008** `src/terezy/data/declarations/resolver.py` — `CandidateDeclarations`,
      `resolve_candidates`, `candidates_from_data_root`.
- [ ] **T009** `data/candidates/owner-001.toml` — the declared ceiling, with the argument for
      its number in the file's own header.
- [ ] **T010** `scripts/check_provenance.py` — `candidates` in `EXEMPT_DIRS`, with its reason.
...
- [ ] **T011 (test)** `tests/unit/test_candidate_records.py` — the shapes are frozen, the two
...
- [ ] **T012** `src/terezy/core/results/candidates.py` — every record in
...
- [ ] **T013 (test)** `tests/worked_examples/test_candidate_enumeration.py` — the shipped
      ...
      hard-coded (SC-001).
- [ ] **T014** `src/terezy/core/decision/candidates.py` — `enumerate_candidates`: the pair
...
- [ ] **T015 (test)** `tests/unit/test_no_candidate_column.py` — FR-013 versus FR-014 by
      **record**; compose's words carried verbatim (SC-008); a pair in the second column is
      never in the drop count.
- [ ] **T016 (test)** `tests/unit/test_candidate_order.py` — SC-003 (same question twice; files
...
- [ ] **T017 (test)** `tests/unit/test_candidate_refusals.py` — SC-009 (ceiling one above, zero
...
- [ ] **T018 (test)** `tests/unit/test_identity_exit_candidate.py` — SC-018 on a fixture
...
- [ ] **T019 (test)** `tests/unit/test_candidate_marks.py` — SC-014: the set reports the
...
- [ ] **T020 (test)** `tests/worked_examples/test_candidate_accounting.py` — both identities of
      FR-009 by hand on the shipped registry, with the arithmetic beside the assertion.
- [ ] **T021** `src/terezy/core/decision/candidates.py` — `survey`, `evaluated`, `dropped`,
      `drop_tally`, and the two survey-only refusals.
- [ ] **T022 (test)** `tests/unit/test_drop_tally.py` — SC-007: the tally recomputed from the
...
- [ ] **T023 (test)** `tests/invariants/test_candidate_accounting.py` — SC-004: both identities
      over generated registries and questions, covering all-dropped, all-yielding-nothing, and
      nothing declared at all.
- [ ] **T024 (test)** `tests/unit/test_candidate_survey.py` — SC-015 (benchmark index points at
      ...
      the same key).
- [ ] **T025 (test)** `tests/unit/test_candidate_regimes.py` — SC-013: two regimes, keys equal
...
- [ ] **T026 (test)** `tests/unit/test_seventeen_refusals_through_the_loop.py` — SC-005. One
...
- [ ] **T027 (test)** `tests/contract/test_candidates_construct_nothing.py` — SC-019: every
...
- [ ] **T028 (test)** `tests/contract/test_candidates_add_no_rule.py` — SC-006 (no feasibility
...
- [ ] **T029 (test)** `tests/contract/test_candidate_question_travels.py` — SC-012, SC-017,
      ...
      no population.
- [ ] **T030 (test)** `tests/golden/test_candidate_set.py` — the shipped set as a checked-in
...
- [ ] **T031** `docs/METHODOLOGY.md` — a new `## 32` before *Where to look next*: what a
...
- [ ] **T032** `docs/REQUIRED_TESTS.md` — flip **I1** with its test path and the note that
...
- [ ] **T033** `specs/features.toml` — add to the `[[future]]` notes of
