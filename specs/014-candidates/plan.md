...
| **V — Test-first** | **PASS.** Each module lands after a test that fails without it. The shipped set's nine candidates are a golden **and** are derived from the registry inside the test, so a golden that stopped matching the declarations fails on the derivation rather than freezing the number. |
