...
- [ ] **T001** `tests/official_rates.py` — synthetic fixtures: a series builder, an
...
- [ ] **T002** `tests/worked_examples/test_official_rate_base.py` — SC-001, SC-002, SC-012:
  hand arithmetic checked in beside each assertion, including a series quoting per 100 units.
  Fails with `ImportError`.
- [ ] **T003** `tests/unit/test_official_rate_refusals.py` — SC-003: a gap in the middle, a
...
- [ ] **T004** `src/terezy/core/tax/official_rate.py` — the records, `strike_base`,
  `observation_for`, `covered_window`, `provenance_of`, and the two refusals. Makes T002–T003
  pass.
- [ ] **T005** `tests/unit/test_official_rate_rule.py` — the enumerated rule applied in the
...
- [ ] **T006** `tests/contract/test_official_rate_declaration_loading.py` — SC-004's battery of
...
- [ ] **T007** `data/observation_kinds.toml` — the `official_rate` kind, 7 days, with its note.
- [ ] **T008** `src/terezy/data/declarations/schema.py` — `OfficialRateSeriesTable`,
...
- [ ] **T009** `src/terezy/data/declarations/loader.py` — `official_rate_from_file`, every
  refusal naming file and field; `TimingDeclaration` carries the series reference.
- [ ] **T010** `src/terezy/data/declarations/resolver.py` — `OFFICIAL_RATES_DIR`,
...
- [ ] **T011** [P] `scripts/check_provenance.py` — `official_rates` in `SOURCED_DIRS`,
  `quotation_unit` in `STRUCTURAL_KEYS`, each with its reason in the comment beside it.
- [ ] **T012** `data/official_rates/ua_nbu_usd.toml` — identity only. Header states: no
...
- [ ] **T013** `data/tax/timing/ua.toml` — `official_rate_series = "ua_nbu_usd"`.
- [ ] **T014** `tests/contract/test_official_rate_data_only.py` — SC-011: a second series with
...
- [ ] **T015** `tests/contract/test_ua_series_refuses.py` — SC-014, against the **shipped**
...
- [ ] **T016** `tests/unit/test_tax_base_in_the_tax_currency.py` — a foreign coupon charge
...
- [ ] **T017** `src/terezy/core/tax/year.py` — `AssessmentRules.official_rate`,
  ...
  `add(pit, levy)`.
- [ ] **T018** `src/terezy/data/declarations/resolver.py` — `tax_rules_from_data_root` passes
  the resolved series into `AssessmentRules`.
- [ ] **T019** `tests/contract/test_official_rate_marks.py` — SC-005 and SC-006: an unverified
...
- [ ] **T020** `tests/contract/test_tax_declaration_loading.py` — update
...
- [ ] **T021** `src/terezy/core/results/tuple.py`, `src/terezy/core/decision/tuple_outcome.py`
...
- [ ] **T022** `.importlinter` — two contracts,
  `official-rate-never-prices-a-leg` (FR-012) and `no-tax-base-from-a-channel` (FR-013).
- [ ] **T023** `tests/contract/test_architecture_boundaries.py` — both contract names added to
  the pinned list, so deleting one is a test failure.
- [ ] **T024** `tests/contract/test_the_rate_you_are_taxed_at.py` — SC-007 (one dollar amount
...
- [ ] **T025** `docs/METHODOLOGY.md` — a new section: what the tax base is in plain language,
...
- [ ] **T026** `docs/REQUIRED_TESTS.md` — F1's note narrowed to its remaining blocker; F2 and
  F3 left open with what 011 established and what it did not; F5 untouched.
- [ ] **T027** `specs/features.toml` — 011 `in-progress` then `done` at landing;
  `fx-tax-asymmetry-f1`'s note narrowed.
- [ ] **T028** `specs/011-official-rate/quickstart.md`.

## Phase 6 — Close

- [ ] **T029** Full gates: `ruff check` + `ruff format --check`, `mypy`, `pytest --cov`,
...
- [ ] **T030** `/condense` over the branch diff, then `/code-review` until clean.
