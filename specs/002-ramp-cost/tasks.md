...
- [ ] T019 [US1] Implement `src/terezy/core/routes/ranking.py` — `rank` costing every candidate with `cost_one` and returning `Ranking(costed, recommended: int, excluded, ties, not_comparable)`. Closes **FR-016**, **FR-029**
...
- [ ] T029 [US3] Implement `src/terezy/core/routes/capacity.py` and add the accumulator to `LedgerState` — keyed by `(route_id, year, month)` taken from each event's `occurred_on`, **never a clock**. Confirm C1–C6 still pass: this is new state in the fold. Closes **FR-012**
