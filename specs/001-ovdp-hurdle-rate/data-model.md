...
| `occurred_on` | `date` | |
| `kind` | `EventKind` | `purchase`, `coupon`, `principal_repayment`, `tax_charge`, `reinvestment`, `cash_deposit`, `fee`, `disposal`, and ⚙ `ramp_movement` from feature 002 — no existing kind could carry a currency crossing, and `cash_deposit` means money from outside the modelled system. Partitioned by the `frozenset` constants `LOT_OPENING_KINDS`, `LOT_CLOSING_KINDS`, `CASH_ONLY_KINDS`, so the partition is inspectable. |
...
| `caused_by` | `CausationRef` | Required, never optional. `CausationKind` has **no catch-all member**: every member names a *kind of declaration*. Feature 001 had exactly two — `INSTRUMENT_TERM` and `TAX_RULE` — and feature 002 added `ROUTE_TERM`, because a ramp fee is charged by neither and without a third member it would have to claim a cause it does not have. ⚙ The closure that matters is the absence of a bucket, not the count: `SYSTEM` or `OWNER_ACTION` remain forbidden, because they are where every event whose cause nobody tracked down would land and C6 would then pass while meaning nothing. `ROUTE_TERM` resolves to a route id, and C6 asserts the mapping from event kind to causation kind rather than merely that a cause is present. |
