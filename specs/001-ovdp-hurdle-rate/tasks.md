...
- [x] T024 [US1] Write `tests/worked_examples/test_ovdp_schedule.py` — the D1 example: a stated purchase of a synthetic issue held to maturity, with the full coupon and principal schedule computed by hand and checked in beside the assertion, and **total tax exactly zero**. Closes **D1**, SC-001, SC-002. Must fail first
- [x] T025 [US1] Implement `src/terezy/core/instruments/interface.py` — `EventsFn`, `TaxClassesFn`, `ConstraintsFn` type aliases, frozen `InstrumentOps` record, and `REGISTRY: Mapping[str, InstrumentOps]`. Unknown `instrument_class` is a failure, never a fallback
- [x] T026 [US1] Implement `src/terezy/core/instruments/fixed_income.py` — free functions computing the closed-form coupon and principal schedule from `BondTerms` via the declared conventions, exported as `OPS`. Returns `InconsistentTerms` when maturity ≤ issue and `InfeasiblePurchase` below `min_ticket`. Closes FR-001, FR-018
- [x] T027 [P] [US1] Implement `src/terezy/core/tax/interface.py` — `ChargeFn` alias, frozen `TaxRuleOps`, `REGISTRY`. The rule takes `TaxClass` as an argument rather than closing over it
- [x] T028 [P] [US1] Implement `src/terezy/core/tax/flat_rate.py` — applies whatever `pit_rate` and `levy_rate` the declared class carries, PIT and levy on **separate bases** and reported as separate lines. A zero charge is still a charge, carrying the class's provenance. **No exempt-specific branch exists.** Closes FR-003
- [x] T029 [US1] Implement `src/terezy/core/results/schedule.py` — `CashFlowSchedule` derived from ledger events, each row carrying date, gross, tax, net and the convention that placed the date
- [x] T030 [US1] Implement `src/terezy/core/results/hurdle.py` — `HurdleRate` with `nominal_ytm`, `nominal_cash_flow_return`, `real: RealRate | RealTermsUnavailable`, `total_tax`, `excludes`, `provenance`. Closes FR-004, FR-005, FR-022
- [x] T031 [P] [US1] Write `tests/unit/test_hurdle_real_slot.py` — the real slot is present and explicitly `RealTermsUnavailable` with a reason, never absent and never holding a nominal value. Closes **SC-011**
- [x] T032 [P] [US1] Write `tests/unit/test_infeasible_and_inconsistent.py` — purchase below minimum ticket reports the shortfall and is not rounded; maturity on or before issue produces no schedule; non-positive quantity is rejected. Closes FR-018 and the spec's edge cases
- [x] T033 [US1] Add `src/terezy/core/results/project.py` wiring schedule → events → ledger → `HurdleRate`, and take T024 green. Every figure derives from the ledger, never from the schedule directly (research.md D3)
