...
seven are buildable now: type sync has nothing to compare until the types are generated, and the
...
- [ ] T014 Add **four** jobs to `.github/workflows/ci.yml` — **Typecheck**, **Lint**, **Unit** and **Bundle URLs** — each **unconditional**, not filtered on a `web/` path, and each green on the tree as it stands. **Type sync is deliberately not here**: it has nothing to compare until T043 generates `schema.d.ts`, so adding it now would make it a blocking gate that is red, and Phases 3 to 7 each end in a commit that `/commit` refuses to make while a gate is red. It is added by T044, in the phase that gives it an input.
...
- [ ] T043 **[020]** Add `web` scripts generating `web/src/api/schema.d.ts` from `src/terezy/api/http/openapi.json` with `openapi-typescript`, reading the document **from the local filesystem** by package path, and **commit the output** (FR-003). Verify risk **R2** at this task and nowhere later: each `oneOf` member must carry its tag as a **literal** type, not as `string`. If it does not, FR-004 has no mechanical form and the fix belongs in 020's document — stop and raise it.
- [ ] T044 **[020]** Add the **Type sync** job to `.github/workflows/ci.yml`, immediately after T043 gives it something to compare: regenerate `web/src/api/schema.d.ts` and fail on any difference from the committed output (FR-003, SC-013). **Unconditional**, not filtered on a `web/` path — the job exists to turn red when a *Python* change moves the OpenAPI document, and a path filter would disable it on the only change it was built for.
...
- [ ] T047 **[020]** Write SC-003's demonstration in `web/tests/unit/union-widening.test.ts`: add a member to a **copy** of the OpenAPI document, regenerate into a scratch directory, and assert `tsc` **fails**. This is the only one of FR-004's three mechanisms that cannot pass vacuously, and it is what proves T006 and T007 are actually wired.
...
| **API-bound** | T043–T063 | **21** — the generated types, the type-sync gate, the Playwright suite, packaging, and landing |
