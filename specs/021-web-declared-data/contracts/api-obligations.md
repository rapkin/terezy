...
| | |
|---|---|
| **Input** | what `uv run python scripts/generate_openapi.py` writes — 020 FR-040, generated on the fly and stored nowhere (owner decision 2026-09-05), byte-reproducible across processes (020 FR-039) |
| **Output** | `web/src/api/schema.d.ts`, **gitignored** and regenerated by every build (FR-003) |
| **Gate** | the types-generate CI job generates and then typechecks (SC-013): red if generation fails, and red where the client names a shape the document no longer carries. It is **unconditional**, not filtered on a `web/` path: it exists to turn red when a *Python* change moves the document, and a path filter would disable it on the only change it was built for |
