...
| E11 | A **zero** tax figure distinguishes *exempted* from *not applicable* when rendered. The engine already separates them — a taxable event's zero cites its tax class, a non-taxable row's zero cites nothing because there is nothing to cite — but a reader looking at a schedule table sees `0.00` on every row either way. A presentation requirement for the waterfall (spec §5.3), recorded here so the distinction the engine preserves is not thrown away at the last step. | `[x]` **Closed by feature 027**, which built the surface the row had been waiting for since 001. The distinction is discriminated on whether the amount **cites** something and never on the amount alone (`web/src/card/zeros.ts`, `web/tests/unit/zeros.test.ts`); it renders as two different statements, neither blank (`web/tests/unit/tax-bar.test.tsx`); and both cases are asserted to be real on the shipped registry rather than on a fixture — 119 cited zeros against 63 resting on no source, measured 2026-09-11 (`tests/contract/test_the_card_accounts_for_what_came_back.py`) |
...
| **J4** | Touched and deliberately not claimed. A lock-up longer than the horizon reaches `CannotSpanHorizon`, and this feature makes every such candidate visible in one report rather than one at a time — planted and asserted in `tests/unit/test_every_refusal_through_the_loop.py`. What the row asks for still does not exist: a declared `lock_up_months` term compared against a horizon. 006 did not claim it and neither does this. |
...
| Row | How, and why the box does not move |
|---|---|
| **F2** | *Switching display currency changes no realised amount, no tax figure, and no after-tax UAH ranking.* **Stays open, and the reason is now recorded rather than pending.** The owner deferred the display switch on 2026-09-03, so this feature has no switch to check the row against; the row's own note exists to stop it being closed by a feature that never checked it, and closing it here would be that. `display-currency-switch` in `specs/features.toml` is what it now waits on. |
| **H2** | Reinforced at a new surface: a malformed declaration reaches an HTTP caller carrying the loader's own file, field, problem and remedy, and no category returns partial data (`tests/contract/test_a_broken_declaration_reaches_the_caller.py`). The row's own test stays `tests/contract/test_declaration_loading.py`. |
| **H3** | Untouched. This feature reads declarations aloud; it adds no input the manifest does not already record, and `tests/unit/test_the_registry_summary.py` asserts every manifest input reference appears under exactly one category with the manifest's own digest. |
| **H4** | Reinforced: a new layer arrives and the boundary contract **grows**. `frameworks-only-in-the-http-module` forbids the framework in `data/`, `cli/`, `api/answer.py` and `api/diagrams/`; the two package `__init__` files no contract can name are scanned, and a completeness check asserts the union reaches every module under `src/terezy/` (`tests/contract/test_the_framework_stays_in_one_module.py`). |
...
| **E4** | Pressed on the figure 025 built and nothing rendered: a held position is valued in the **price** currency, and the base-currency restatement the engine already computed is what the screen leads with. Showing the first beside a hryvnia basis read as a 97% loss where the engine had computed a 32% gain — found by the merge's own review round, fixed, and held by `web/tests/unit/held-positions.test.tsx`. The row is the engine's and does not move. |
...
**027-candidate-card** closes **E11**, which had been open since feature 001 — every feature that
touched it recorded the same thing, that the engine keeps the two zeros apart and that no surface
draws them. The surface is the waterfall on a candidate's card, and the distinction is a typed
match on whether the amount **cites** something rather than on its magnitude
(`web/src/card/zeros.ts`, `web/tests/unit/zeros.test.ts`, `web/tests/unit/tax-bar.test.tsx`).
Both cases are real on the shipped registry rather than planted: measured 2026-09-11, 119 of the
122 charges the owner's question strikes are zeros carrying their exemption's citation, and 63
schedule rows carry a zero resting on no source because no rule ran
(`tests/contract/test_the_card_accounts_for_what_came_back.py`).

| Row | How, and why the box does not move |
|---|---|
| **E5** | Pressed at the place a mark is most easily lost and had never been checked: `net = gross - tax` per flow. The sweep asserts that a flow's net carries the **union** of the sources its gross and its tax carry, over every evaluated candidate of the owner's question — an assertion that merely found a provenance record present would pass on an empty one, which is what a dropped mark looks like. |
| **C6** | *Every displayed figure resolves to ledger events and to the rule that produced it.* Reinforced at the surface rather than closed again: every bar is a served flow and every flow carries its `caused_by` — the declaration or the tax rule behind the event — so a reader can follow a figure back without leaving the card. What the row's own test covers and this does not carry is the **ledger** itself, which has no JSON key form and is recorded as absent with its reason (`docs/METHODOLOGY.md` §38). |
