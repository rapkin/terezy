    ...
    return frozenset(
        {
            ...
            "observed quotation is this holding's price at purchase and falls only by the "
            "coupons detaching after it -- neither is a term of the paper"
        }
    )
...
def _sold_early(events: Sequence[Event], early_exit: EarlyExit | None) -> SoldEarly | None:
    ...
    return SoldEarly(
        ...
        # quotation as OBSERVED: the price struck is that quotation net of the coupons that
        # detached before the sale, and a record quoting the pre-detachment figure beside
        # post-detachment proceeds would be two numbers for one trade.
        price_per_unit=money.scale(sale.amount, 1.0 / sale.quantity),
