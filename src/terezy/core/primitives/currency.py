...
Constitution Principle VI gives currency three distinct roles -- **base** (UAH),
**tax** (UAH at the official rate on the transaction date) and **display**
(user-switchable) -- and conflating any two of them is a defect. This enum names the
*denomination* only. It is deliberately not a role: a role is a property of the
scenario, not of the amount, and encoding one here would be the first step towards
exactly the conflation the principle forbids.
...
class Currency(Enum):
    ...
    Enums are welcome under the functional-style rule -- they are closed sum types, not
    objects (constitution, Engineering Standards, owner decision D-E). ``value`` is the
    stable identifier used in canonical form and in declaration files; it is part of the
    data contract and may not be renamed casually.
    """
