...
⚙ **A channel's premium is not on this diagram, and that is a stated limit rather than an
omission.** FR-006 names "fees, premiums", but a premium is declared on an ``FxChannel`` and
this renderer's declared inputs are venues and routes -- it never reads a declaration file of
its own (FR-020), so it renders the per-leg figures the ``Leg`` record carries and no others.
The premium reaches a diagram where a result carries it: on the costed path, as the
spread over reference, labelled as itself and never as the cost.
...
⚙ **``kinds`` and ``as_of`` are on the signature and are not in ``contracts/rendering.md``.**
The contract's parameter list carried neither, and FR-013 cannot be satisfied without both:
staleness is ``as_of - retrieved_on`` against a *declared per-kind* threshold. Making them
optional was rejected -- an unassessed diagram would be indistinguishable from a clean one,
which is the silent permissive default FR-028 forbids and the very ambiguity
``staleness.UNASSESSED`` exists to remove.
"""
...
_MODE_NOTE: Mapping[Mode, str] = {
    ...
    Mode.DECLARED_FIGURES: (
        "declared per-leg fees only -- no computed ramp cost appears on a registry graph"
    ),
}
...
"""The prefix of the one field :attr:`Mode.DECLARED_FIGURES` adds and :attr:`Mode.TOPOLOGY`
omits.

Named as a constant because SC-012's assertion is that the two modes differ *by figures only*:
the test strips every field beginning with this prefix from the with-figures text and requires
what is left to be the topology text, byte for byte. A second figure field added without a
shared prefix would make that assertion silently weaker.
"""
...
def _leg_marks(
    leg: Leg, route: Route, kinds: Mapping[str, ObservationKind], as_of: date
) -> tuple[Mark, ...]:
    """Every mark this leg's edge carries: its epistemic state, plus its route's status."""
    verdict = staleness.staleness_of(
        leg.provenance, kinds, kind=leg.kind_of_observation, as_of=as_of
    )
    found = list(marks.epistemic(leg.provenance, stale=staleness.any_stale(verdict)))
...
def _leg_fields(leg: Leg, route: Route, mode: Mode, applicable: tuple[Mark, ...]) -> list[str]:
    ...
    if mode is Mode.DECLARED_FIGURES:
        fields.append(
            f"{FIGURE_FIELD}{numbers.percent(leg.fee_pct)} + {numbers.amount(leg.fee_fixed)}"
        )
    fields.append(f"status: {route.status}")
    fields.append(marks.segment(applicable, unsourced=marks.is_unsourced(leg.provenance)))
    return fields
...
def render_graph(
    *,
    ...
    synthetic = any(marks.is_synthetic(leg.provenance) for route in drawn for leg in route.legs)
    ...
    for route in drawn:
        for leg in sorted(route.legs, key=lambda item: item.index):
            applicable = _leg_marks(leg, route, kinds, as_of)
            lines.append(
                mermaid.edge(
                    node_of[leg.from_venue],
                    node_of[leg.to_venue],
                    mermaid.label(*_leg_fields(leg, route, mode, applicable)),
