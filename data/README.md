...
| `scenarios/` | FX paths, discrete events, regime transitions, risk assumptions (spec §4.3.4, §4.6). `scenarios/inflation/` holds the owner's declared belief about future inflation, in a subdirectory because `scenarios/*.toml` is globbed as scenario documents and does not recurse — the same reading `instruments/nav/` carries. |
