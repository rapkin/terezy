...
| `observations/` | Dated snapshots of what a source publishes, written by a fetch script. Two of the three are the evidence a declaration was checked against, read by a human and by nothing at run time, kept so a later correction can name what it saw; `binance_<symbol>.toml` is **read at run time** as a dated close series, because a daily close has no judgement in it and hand-promoting one would be transcription (025 FR-011). No such file ships: the fetch writes the owner's own dated retrieval, and a committed price is one nobody re-fetched. A declaration keeps the other two apart: `inzhur.toml` is a **seller's** publication and carries the price, `nbu_depository.toml` is the **issuer's** register through the National Bank and carries the terms. Each is the whole of what its endpoint returned, because filtering to the issues somebody wanted would be a judgement in a fetcher — and an issue a seller lists that the register does not can only be seen against the register's whole membership. |
| `scenarios/` | FX paths, discrete events, regime transitions, risk assumptions (spec §4.3.4, §4.6). `scenarios/inflation/` holds the owner's declared belief about future inflation, and `scenarios/tax/` his filing decisions and his positions on the two tax questions no source answers — both in subdirectories because `scenarios/*.toml` is globbed as scenario documents and does not recurse, the same reading `instruments/nav/` carries. `scenarios/quotation/` holds his belief about what a dated quotation is worth on another day — its clean price holds, and interest accrues linearly inside each declared coupon period — no default, so a run with none refuses at load, and every figure computed through it names it (015 FR-032, 022 FR-017). |
